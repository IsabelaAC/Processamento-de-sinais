...
def create_param_entries(param_frame):
    ...
    tk.Label(param_frame, text="Frequência (CW) / Frequência Inicial (Chirp) (Hz):").grid(row=1, column=0)
    frequency_entry = tk.Entry(param_frame)
    frequency_entry.grid(row=1, column=1)

    tk.Label(param_frame, text="Frequência Final (Chirp) (Hz):").grid(row=2, column=0)
    freq_end_entry = tk.Entry(param_frame)
    freq_end_entry.grid(row=2, column=1)
    
    tk.Label(param_frame, text="Frequência de amostragem (Hz):").grid(row=3, column=0)
...
def create_buttons(button_frame, app):
    tk.Button(button_frame, text="Generate CW Signal", command=app.generate_cw_signal).pack(side=tk.LEFT)
    tk.Button(button_frame, text="Generate Chirp Signal", command=app.generate_chirp_signal).pack(side=tk.LEFT)
    tk.Button(button_frame, text="Compute FFT", command=app.compute_fft).pack(side=tk.LEFT)
    tk.Button(button_frame, text="Show Spectrogram", command=app.show_spectrogram).pack(side=tk.LEFT)
    tk.Button(button_frame, text="Save as WAV", command=app.save_as_wav).pack(side=tk.LEFT)
