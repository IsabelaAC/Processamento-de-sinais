...
# -*- coding: utf-8 -*-
"""
Created on Mon May 21 13:06:07 2024
...
class SignalGeneratorApp:
    def __init__(self, root):
        ...
        self.amplitude_entry, self.frequency_entry, self.freq_end_entry, self.fs_entry, self.duration_entry = create_param_entries(self.param_frame)
    ...
    def get_parameters(self):
        try:
            ...
            fs = float(self.fs_entry.get() or 2 * frequency)
            if fs < 2 * frequency:
             fs = 2 * frequency
            duration = float(self.duration_entry.get() or 1.0)
        except ValueError:
            messagebox.showerror("Erro de Entrada", "A frequência é obrigatória.")
            return None
